...
if [ "$is_uefi" == true ]; then
    echo "size=300M, type=ef" | sfdisk --force $drive_path -A 1
    mkfs.fat -F32 "$drive_path"1
else
    echo "size=200M, type=83" | sfdisk --force $drive_path -A 1
...
echo "size=$swap_size_bytes, type=82" | sfdisk $drive_path -A 2
mkswap "$drive_path"2
swapon "$drive_path"2
